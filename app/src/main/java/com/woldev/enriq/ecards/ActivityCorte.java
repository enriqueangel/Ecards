...
public class ActivityCorte extends AppCompatActivity {
    ...
    private void cargarHoras() throws JSONException {
        ...
        // Adding child data
        List<Corte> SinProblemas = new ArrayList<>();
        List<Corte> ConProblemas = new ArrayList<>();
        ...
        for (int i = 0; i < CargarUsuarios.length(); i++) {
            ...
            if (IsNegativo || TiempoTemp.equals("00:00")){
                SinProblemas.add(new Corte(NombreMostrarTemp, TiempoTemp));
            }else{
                ConProblemas.add(new Corte(NombreMostrarTemp, TiempoTemp));
            }
                    ...
                    JsonObjectRequest arrReq = new JsonObjectRequest(Request.Method.POST, urltemp, new JSONObject(params),
                            new Response.Listener<JSONObject>() {
                                ...
                                public void onResponse(JSONObject response) {
                                    try {
                                        ...
                                        dialog.dismiss();
                                        Toast.makeText(ActivityCorte.this, "Error cargando el corte", Toast.LENGTH_SHORT).show();
                            ...
                            new Response.ErrorListener() {
                                ...
                                public void onErrorResponse(VolleyError error) {
                                    dialog.dismiss();
                                    Log.e("Volley", error.toString());
                                    Toast.makeText(ActivityCorte.this, "Error en la conexion.", Toast.LENGTH_SHORT).show();
                        ...
                        public Map<String, String> getHeaders() throws AuthFailureError {
                            HashMap<String, String> headers = new HashMap<String, String>();
                            headers.put("Content-Type", "application/json; charset=utf-8");
                            SharedPreferences SP = ActivityCorte.this.getSharedPreferences("TOKEN",MODE_PRIVATE);
                            String tokenTemp = SP.getString("token","");
                            headers.put("token", tokenTemp);
                            return headers;
                        }
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        ...
        switch (item.getItemId()){
            case android.R.id.home:
                ...
                return true;
            case R.id.historial:
                i = new Intent(ActivityCorte.this, ActivityHistorialCorte.class);
